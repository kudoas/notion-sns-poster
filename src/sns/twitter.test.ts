...
import { TwitterPoster } from './twitter.ts'

// Mock global fetch
const mockFetch: ReturnType<typeof mock> = mock(() => Promise.resolve())
...
describe('TwitterPoster', () => {
  ...
  describe('postArticle', () => {
    ...
    test('should throw error when Twitter API returns error status', async () => {
      ...
      await expect(twitterPoster.postArticle(mockArticle)).rejects.toThrow('Failed to post article error-article to Twitter: 403 Forbidden')
    })
