...
const mockDetectFacets = mock(() => Promise.resolve());
...
describe("BlueskyPoster", () => {
  ...
  beforeEach(() => {
    blueskyPoster = new BlueskyPoster(
      mockCredentials.identifier,
  ...
  describe("postArticle", () => {
    test("should successfully post article to Bluesky", async () => {
      ...
      await expect(
        blueskyPoster.postArticle(mockArticle)
      ...
      expect(postCall.text).toBe(
        "🔖 Test Article Title https://example.com/test-article"
      );
    ...
    test("should handle login errors", async () => {
      ...
      await expect(blueskyPoster.postArticle(mockArticle)).rejects.toThrow(
        "Invalid credentials"
      );
    ...
    test("should handle articles with special characters", async () => {
      ...
      const postCall = mockPost.mock.calls[0][0];
      expect(postCall.text).toBe(
        '🔖 Article with émojis 🚀 and "quotes" https://example.com/special?param=value&other=test'
      );
    });
